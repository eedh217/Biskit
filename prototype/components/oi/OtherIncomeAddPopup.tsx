...
export default function OtherIncomeAddPopup({ paymentYear, paymentMonth, onClose, onSaved }: Props) {
  ...
  const validateAttrDateFields = () => {
    if (attrYear && attrMonth) {
      const err = validateAttrDate(Number(attrYear), Number(attrMonth), paymentYear, paymentMonth);
  ...
  const handleSubmit = () => {
    ...
    if (attrYear && attrMonth) {
      const dateErr = validateAttrDate(Number(attrYear), Number(attrMonth), paymentYear, paymentMonth);
