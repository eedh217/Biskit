...
import { addWorkType } from "@/lib/store";
...
export default function WorkTypeAddPopup({ onClose, onSuccess }: Props) {
  const [name, setName] = useState("");
  const [code, setCode] = useState("");
  const [errors, setErrors] = useState<FieldError>({});

  const validateField = (field: "name" | "code", value: string): string | undefined => {
    if (!value.trim()) return undefined; // blur에서 빈값은 submit에서 처리
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const result = addWorkType(name.trim(), code.trim());
    if (!result.success) {
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">근로형태 추가</h2>
