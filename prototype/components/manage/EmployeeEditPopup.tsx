...
import { updateEmployee, getWorkTypes } from "@/lib/store";
...
export default function EmployeeEditPopup({ employee, onClose, onSuccess }: Props) {
  const workTypes = getWorkTypes();

  const [form, setForm] = useState({ ...employee });
  const [idType, setIdType] = useState<"foreign" | "passport">(
    employee.passportNumber ? "passport" : "foreign"
  );
  ...
  const handleNationalityChange = (val: "내국인" | "외국인") => {
    setForm((prev) => ({
      ...
      passportNumber: "", birthDate: "", gender: undefined,
      country: "",
    }));
    setErrors({});
  };
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!result.success) {
      if (result.error?.includes("이메일")) newErrors.email = result.error;
      else newErrors.employeeNumber = result.error;
      setErrors(newErrors);
      return;
    }
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white flex items-center justify-between px-6 py-4 border-b border-gray-200 z-10">
          <h2 className="text-lg font-semibold text-gray-900">임직원 수정</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={form.employeeNumber} readOnly className="w-full px-3 py-2 border border-gray-200 rounded-md text-sm bg-gray-50 text-gray-500 cursor-not-allowed" />
            <p className="mt-1 text-xs text-gray-400">사번은 수정할 수 없습니다.</p>
          </div>
          ...
          {form.nationality === "내국인" && (
            <div>
              ...
              <div className="flex items-center gap-2">
                <input type="text" value={form.residentNumber1} onChange={(e) => set("residentNumber1", e.target.value.replace(/\D/g, ""))} maxLength={6} placeholder="앞 6자리" className={smallInputClass("residentNumber1") + " w-32"} />
          ...
          <div>
            ...
            <input type="text" value={form.phone} onChange={(e) => set("phone", e.target.value.replace(/[^0-9+()-]/g, ""))} maxLength={30} className={inputClass("phone")} />
