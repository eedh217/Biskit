...
import { getPayItems, deletePayItems } from "@/lib/store";
...
import PayItemAddPopup from "@/components/manage/PayItemAddPopup";
import PayItemEditPopup from "@/components/manage/PayItemEditPopup";

export default function PayItemListPage() {
  const [allData, setAllData] = useState<PayItem[]>([]);
  const [filtered, setFiltered] = useState<PayItem[]>([]);
  ...
  const [editTarget, setEditTarget] = useState<PayItem | null>(null);
  ...
  const applySearch = (data: PayItem[], query: string) => {
    if (!query.trim()) {
      setFiltered(data);
    } else {
      const q = query.trim().toLowerCase();
      setFiltered(data.filter((p) => p.payCode.toLowerCase().includes(q) || p.itemName.toLowerCase().includes(q)));
    }
    setCurrentPage(1);
  };
  ...
  const handleDelete = () => {
    ...
    setConfirm({
      ...
      onConfirm: () => {
        const result = deletePayItems([...selectedIds]);
        setConfirm(null);
        if (result.failedIds && result.failedIds.length > 0) {
          const failedItems = allData.filter((p) => result.failedIds!.includes(p.id)).map((p) => ({
            name: `${p.itemName} (${p.payCode})`,
            reason: "확정된 급여대장에서 사용 중",
          }));
  ...
  const handleExcelDownload = () => {
    ...
    const header = "구분,급여코드,항목명,유형";
    const rows = target.map((p) => `${p.category},${p.payCode},${p.itemName},${p.itemType}`);
    const csv = "\uFEFF" + [header, ...rows].join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "HRIS_급여항목_리스트.csv";
    a.click();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">급여항목 관리</h1>
        <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-600">급여코드</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600">항목명</th>
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                      p.category === "지급항목" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                    }`}>
                      {p.category}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-700 font-mono">{p.payCode}</td>
                  <td className="px-4 py-3 text-gray-900">{p.itemName}</td>
                  <td className="px-4 py-3 text-gray-700">{p.itemType}</td>
                </tr>
              ))
            )}
          </tbody>
