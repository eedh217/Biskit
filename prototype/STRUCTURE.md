...
| 파일 | 역할 |
|------|------|
| `store.ts` | localStorage CRUD 작업, 귀속예외 로직 처리 |
| `taxCalculation.ts` | 세율 결정, 소득세/지방소득세 계산 |
| `spsValidation.ts` | 주민/사업자등록번호 검증, 체크디지트 알고리즘 |
| `industryCodes.ts` | 40개 업종코드 정의, 예외업종 상수 |
| `formatUtils.ts` | 금액/날짜 포맷팅 유틸리티 |
| `excelUpload.ts` | 엑셀 업로드 파싱 및 검증 |
| `excelTemplate.ts` | 엑셀 다운로드 템플릿 생성 |

### 📌 정책 관련

| 파일 | 역할 |
|------|------|
| `spsPolicyConfig.ts` | SPS 정책 탭 설정 |
...
| 파일 | 역할 |
|------|------|
| `sps.ts` | BusinessIncome, MonthlySummary 등 SPS 타입 |
| `manage.ts` | 관리 화면용 타입 |
| `index.ts` | 공통 타입 정의 |
...
| 월별 리스트 화면 | `app/sps/monthly/page.tsx` | `store.ts`, `formatUtils.ts` |
